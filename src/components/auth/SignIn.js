...
const SignIn = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(`The name you entered was: ${name} and ${password}`);
  };
  return (
    <div className="w-full max-w-xl max-h-full m-auto rounded py-10 px-5 mt-20">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <input
            className="w-full bg-indigo-700 hover:bg-blue-700 text-white font-bold py-2 px-4 mt-6 mb-6 rounded"
            type="submit"
          />
        </div>
      </form>
      ...
      <footer>
        ...
        <a className="text-indigo-700 hover:text-indigo-400 font-bold text-sm float-right">
          <Link to="/signout">Don't have an Account?</Link>
        </a>
      </footer>
    </div>
  );
